use crate::extension::commands::{ExtensionCommands, ExtensionResponses, ExtensionMetadata, Hook};
use crate::Track;
use std::error::Error;
...
impl ExtensionBuilder {
    pub fn new<I: Into<String>, N: Into<String>, V: Into<String>>(id: I, name: N, version: V) -> Self {
        Self {
            id: id.into(),
            name: name.into(),
    ...
    pub fn on_add_to_queue(self, hook: &'static dyn Fn(Vec<Track>) -> Result<Vec<Track>, Box<dyn Error>>) -> Self {
        self.add_hook(HookType::AddToQueue(Box::new(hook)))
    }

    fn add_hook(mut self, hook: HookType) -> Self {
        self.hooks.push(ClientHook {
            hook
        });
        self
    }
...
//    loop {
        ...
        let response = match command {
            ExtensionCommands::Load =>
                ExtensionResponses::Load(ExtensionMetadata {
                    name: extension.name,
                    id: extension.id,
                    version: extension.version,
                    hooks: extension.hooks.iter()
                        .map(|hook| hook.into())
                        .collect()
                })
        };
