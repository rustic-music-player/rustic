...
pub(crate) fn setup_client(
    ...
    let client: Box<dyn RusticApiClient> = {
        let client =
            rustic_native_client::RusticNativeClient::new(Arc::clone(app), extensions, cred_store);
        Box::new(client)
    };
    Arc::new(client)
}

pub(crate) fn setup_remote_client(
    ...
    let client: Box<dyn RusticApiClient> = match config {
        ...
        ClientConfig::Native if options.connect.is_some() => {
            let client = rustic_native_http_client::RusticNativeHttpClient::new(
                options.connect.as_ref().unwrap(),
            );
            Box::new(client)
        }
        #[cfg(feature = "http-client")]
