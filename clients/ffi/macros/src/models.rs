...
fn to_type(ty: &StructFieldType) -> TokenStream {
    match &ty {
        ...
        StructFieldType::Type(ty) if ty == "HashMap < String, MetaValueModel >" => {
            quote! { ::std::collections::HashMap<String, FFIMetaValueModel> }
        }
...
fn field_conversion(name: &str, decl: &StructField) -> TokenStream {
    ...
    match &decl.ty {
        ...
        StructFieldType::Type(ty) if ty == "HashMap < String, MetaValueModel >" => {
            quote! { #name: model.#name.into_iter().map(|(k, v)| (k, v.into())).collect(), }
        }
